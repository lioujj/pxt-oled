...
namespace lumexoled {
    ...
    function checkSending(check: boolean): void {
        if (check) {
            let myTime = input.runningTime()
            while (sending) {
                if ((input.runningTime() - myTime) > 3000)
                    break
            }
            sending = false
        }
    ...
    export function OLED_setTransition(transition: transitionType, time: number): void {
        ...
        if ((transition == 0) || (transition == 1))
            basic.pause((time + 30) * 64)
        else
            basic.pause((time + 30) * 128)
    }
    ...
    export function OLED_setSecTransition(transition: transitionType, x0: number, y0: number, x1: number, y1: number, time: number): void {
        ...
        serial.writeBuffer(myBuff6)
        if ((transition == 0) || (transition == 1))
            basic.pause((time + 30) * (y1 - y0))
        else
            basic.pause((time + 30) * (x1 - x0))
    }
